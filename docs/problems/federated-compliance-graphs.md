...
1. **Conflicting authority mappings are both valid; federation reinforces jurisdiction.** Same decision as cross-framework mapping: divergent mappings from different authorities are both correct within their respective jurisdictions. Federation makes this cleaner — graph boundaries themselves delineate areas of responsibility. The structure reinforces the governance rather than working against it.

2. **All three trust mechanisms available; governance is configurable and auditable.** Pure voting, authority-weighted voting, and designated arbiters are all valid mechanisms. The governance model is configurable per commons instance. Every acceptance decision records who decided, under what authority, and with what mechanism. The governance model must be explicit and auditable, not implicit. Same answer applies to the community mapping commons governance question.

3. **Air-gapped sync: deferred.** The specific mechanisms for selective sync between air-gapped deployments and a commons (snapshot export/import, selective replication by framework, integrity verification without connectivity) are deferred pending implementation experience.

4. **Query performance: deferred.** Federation query performance (latency across sovereign boundaries, caching strategy, query planning) is implementation-level and will vary by graph provider, underlying database, and hardware. Deferred to implementation.

5. **Least privilege metadata exposure; graph navigation is opportunistic.** Federated nodes expose the minimum metadata required to participate. All graph navigation is opportunistic — you discover what you are allowed to see, nothing more. Specific metadata exposure rules are deferred until the schema solidifies, but the default posture is least privilege.
